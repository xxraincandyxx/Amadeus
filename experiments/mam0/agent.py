...
RAG = RAGStore(WORKDIR / "rag_index.json")

# === SECTION: system_prompt ===
SYSTEM = f"""You are a coding agent at {WORKDIR}. Use tools to solve tasks.
Prefer task_create/task_update/task_list for multi-step work. Use TodoWrite for short checklists.
Use task for subagent delegation. Use load_skill for specialized knowledge.
Use rag_ingest to index documents, rag_query to search them, rag_list to see what's indexed, rag_delete to remove.
...
TOOL_HANDLERS = {
    ...
    "rag_ingest":       lambda **kw: RAG.ingest(text=kw.get("text", ""), path=kw.get("path", ""), document_id=kw.get("document_id"), chunk_size=kw.get("chunk_size", 1200), chunk_overlap=kw.get("chunk_overlap", 200)),
    "rag_query":        lambda **kw: RAG.query(query_text=kw.get("query_text", ""), top_k=kw.get("top_k", 5)),
    "rag_list":         lambda **kw: RAG.list_documents(),
    "rag_delete":       lambda **kw: RAG.delete_document(kw.get("document_id", "")),
}
...
TOOLS = [
    ...
    {"name": "claim_task", "description": "Claim a task from the board.",
     "input_schema": {"type": "object", "properties": {"task_id": {"type": "integer"}}, "required": ["task_id"]}},
    {"name": "rag_ingest", "description": "Ingest a file or text into the RAG search index.",
     "input_schema": {"type": "object", "properties": {"path": {"type": "string"}, "text": {"type": "string"}, "document_id": {"type": "string"}, "chunk_size": {"type": "integer"}, "chunk_overlap": {"type": "integer"}}}},
    {"name": "rag_query", "description": "Search ingested documents using keyword retrieval.",
     "input_schema": {"type": "object", "properties": {"query_text": {"type": "string"}, "top_k": {"type": "integer"}}, "required": ["query_text"]}},
    {"name": "rag_list", "description": "List all documents in the RAG index.",
     "input_schema": {"type": "object", "properties": {}}},
    {"name": "rag_delete", "description": "Delete a document from the RAG index.",
