...
pub fn session_log_to_scenario(log: &SessionLog) -> ScenarioDefinition {
    ...
    let mut current = ScenarioStepDef {
        delay_ms: None,
        events: Vec::new(),
        error: None,
    };
    ...
    let close = |step: &ScenarioStepDef, reason: &str| -> ScenarioStepDef {
        ...
        s.events.push(StreamEventDef::StopReason {
    ...
    for event in &log.timeline {
        let RecordedEvent::AgentEvent { event } = &event.event_type else {
            continue;
        };
        started = true;
        match event {
            AgentEventData::TextDelta { delta } => {
                current.events.push(StreamEventDef::TextDelta {
                    text: delta.clone(),
                });
            }
            AgentEventData::ThinkingDelta { delta } => {
                current.events.push(StreamEventDef::ThinkingDelta {
                    text: delta.clone(),
                });
            }
            ...
            AgentEventData::ToolStart { id, name, .. } => {
                current.events.push(StreamEventDef::ToolCallStart {
                    id: id.clone(),
                    name: name.clone(),
                });
            }
            AgentEventData::ToolInputDelta { delta, .. } => {
                current.events.push(StreamEventDef::ToolCallDelta {
            ...
            AgentEventData::ToolComplete { id, input, .. } => {
                ...
                current
                    .events
                    .push(StreamEventDef::ToolCallDone { id: id.clone() });
                steps.push(close(&current, "tool_use"));
                current = ScenarioStepDef {
                    delay_ms: None,
                    events: Vec::new(),
                    error: None,
                };
...
mod tests {
    ...
    fn single_text_turn_becomes_one_step() {
        ...
        log.timeline.push(agent(AgentEventData::TextDelta {
            delta: "Hi".to_string(),
        }));
        log.timeline.push(agent(AgentEventData::Done {
            text: "Hi".to_string(),
            tool_call_count: 0,
        }));
        ...
        assert!(def.steps[0]
            .events
            .iter()
            .any(|e| matches!(e, StreamEventDef::TextDelta { text } if text == "Hi")));
        assert!(def.steps[0]
            .events
            .iter()
            .any(|e| matches!(e, StreamEventDef::StopReason { reason } if reason == "end_turn")));
    }
    ...
    fn tool_turn_splits_into_two_steps() {
        ...
        assert!(def.steps[0]
            .events
            .iter()
            .any(|e| matches!(e, StreamEventDef::ToolCallStart { name, .. } if name == "bash")));
    }

    #[test]
    fn thinking_is_preserved() {
        let mut log = empty_log();
        log.timeline.push(agent(AgentEventData::ThinkingDelta {
            delta: "hmm".to_string(),
        }));
        log.timeline.push(agent(AgentEventData::TextDelta {
            delta: "ok".to_string(),
        ...
        assert!(def.steps[0]
            .events
            .iter()
            .any(|e| matches!(e, StreamEventDef::ThinkingDelta { text } if text == "hmm")));
    }
    ...
    fn on_disk_fixture_round_trips_through_convert_session() {
        ...
        assert!(
            !def.steps.is_empty(),
            "fixture should yield at least one step"
        );
