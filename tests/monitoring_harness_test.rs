...
use scenarios::{
  ...
  assert_timeline_has_thinking,
  assert_timeline_has_token_usage,
  assert_timeline_history_len,
  assert_timeline_history_roles,
  assert_timeline_is_done,
  assert_timeline_text_contains,
  assert_timeline_thinking_contains,
  assert_timeline_tool_count,
  assert_timeline_tool_names,
  ScenarioBuilder,
  ScenarioRunner,
};
...
async fn test_monitoring_timeline_exposes_request_and_history_details() {
  ...
  assert_eq!(timeline.tool_input_for("tool_1"), json!({"path": "Cargo.toml"}).to_string());
...
async fn test_monitoring_timeline_exposes_missing_default_policy_monitoring_gap() {
  ...
  assert!(!timeline.has_approval_requests(), "default test harness policy did not emit approval");
