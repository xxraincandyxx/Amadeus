...
async fn test_file_system_structure_creation() {
    ...
    let tmp_dir = TmpDirBuilder::new("test_fs")
    ...
    // Verify content
    let content = tmp_dir.read_file("project/README.md").await.expect("Failed to read");
...
async fn test_fixture_loader() {
    ...
    if loader.exists("simple.rs") {
        let content = loader.load_string("simple.rs").expect("Failed to load fixture");
        assert!(content.contains("fn main"));
    }

    // Load a JSON fixture
    if loader.exists("sample.json") {
        let json: serde_json::Value = loader.load_json("sample.json").expect("Failed to load JSON");
...
async fn test_tmp_dir_builder() {
    let tmp_dir = TmpDirBuilder::new("test_prefix")
        .build()
        .await
        .expect("Failed to create temp dir");

    // Write a file
    tmp_dir.write_file("test.txt", "Hello, World!").await.expect("Failed to write");
...
async fn test_create_tmp_dir_helper() {
    ...
    let tmp_dir = create_tmp_dir(&files).await.expect("Failed to create tmp dir");
...
async fn test_file_comparison() {
    ...
    let tmp_dir = TmpDirBuilder::new("test_compare")
        .build()
        .await
        .expect("Failed to create temp dir");

    // Create two identical files
    tmp_dir.write_file("file1.txt", "same content").await.expect("Failed to write");
    tmp_dir.write_file("file2.txt", "same content").await.expect("Failed to write");
    tmp_dir.write_file("file3.txt", "different content").await.expect("Failed to write");
    ...
    // Test files_equal
    assert!(files_equal(&path1, &path2).await.expect("Failed to compare"));
    assert!(!files_equal(&path1, &path3).await.expect("Failed to compare"));

    // Test file_content_equals
    assert!(file_content_equals(&path1, "same content").await.expect("Failed to compare"));
    assert!(!file_content_equals(&path1, "different").await.expect("Failed to compare"));
}
...
async fn test_sample_rust_project() {
    ...
    // Create it
    let tmp_dir = TmpDirBuilder::new("rust_project")
    ...
    let cargo = tmp_dir.read_file("project/Cargo.toml").await.expect("Failed to read");
